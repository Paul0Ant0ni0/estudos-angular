...
export class ExoplanetsListComponent implements OnInit {
  ...
  public exoplants: Exoplanet[] = [];

  constructor(private exoPlanetService: ExoplanetService){

  }

  displayedColumns: string[] = ['nome', 'estrelas', 'planetas', 'luas', 'anos', 'distancia'];
  dataSource = new MatTableDataSource<Exoplanet>(this.exoplants);
  ...
  ngOnInit(){
    ...
    this.exoPlanetService.getAll().subscribe(exoplants => {
      this.exoplants = exoplants
      this.dataSource = new MatTableDataSource<Exoplanet>(this.exoplants);
      if(this.paginator) {
